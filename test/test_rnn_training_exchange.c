...
int main(int argc, char* argv[])
{
	...
	else
	{
		...
		iResult = ann_config_set_hidden_layers(cfg, HIDDEN_LAYERS);
		if(iResult != ANN_NO_ERROR)
		{
		...
		{
			iResult = ann_config_set_hidden_nodes(cfg, i, HIDDEN_NODES);
			if(iResult != ANN_NO_ERROR)
			{
		...
		if(iResult != ANN_NO_ERROR)
		{
			printf("ann_config_set_transfer_func_of_layer() failed with error: %d\n", iResult);
			return -1;
		}
	...
	iResult = rnn_bptt_set_max_timestep(ann, MAX_TIMESTEP);
	if(iResult != ANN_NO_ERROR)
	{
		printf("rnn_bptt_set_max_timestep() failed with error: %d\n", iResult);
		return -1;
	}
	...
	while(iterCount < ITER_COUNT)
	{
		...
		*/
		...
		{
			// Forward computation
			rnn_forward_computation(ann, &dataset[i], outputList);
	...
	iResult = rnn_export(ann, "./exchange.rnn");
	if(iResult != ANN_NO_ERROR)
	{
