...
int ann_parse_config_tfunc(struct ANN_CONFIG_STRUCT* cfgPtr,
                           struct ANN_FILE_BLOCK* fbPtr);
int ann_parse_config_topology(struct ANN_CONFIG_STRUCT* cfgPtr,
                              struct ANN_FILE_BLOCK* fbPtr);
int ann_parse_config_training_info(struct ANN_CONFIG_STRUCT* cfgPtr,
                                   struct ANN_FILE_BLOCK* fbPtr);
int ann_parse_config_total_node(struct ANN_CONFIG_STRUCT* cfgPtr,
                                struct ANN_FILE_BLOCK* fbPtr);
...
int ann_parse_recurrent_weight(struct ANN_STRUCT* asPtr,
                               struct ANN_FILE_BLOCK* fbPtr);
...
int ann_parse_threshold(struct ANN_STRUCT* asPtr, struct ANN_FILE_BLOCK* fbPtr)
{
    ...
    // Checking
    iResult =
        ann_strcmp(fbPtr->header, ann_file_header[ANN_HEADER_THRESHOLD_VALUE]);
    if (iResult != ANN_NO_ERROR)
    {
        retValue = ANN_INFO_NOT_FOUND;
        goto RET;
    ...
    for (i = 1; i < fbPtr->strCount; i++)
    {
        ...
        iResult =
            ann_str_extract(&childStrList, &childStrCount, strList[0], '-');
        if (iResult != ANN_NO_ERROR)
        {
            retValue = iResult;
            goto RET;
        ...
        // Set threshold
        iResult = ann_set_threshold_struct(asPtr, layerIndex - 1, nodeIndex - 1,
                                           tmpValue);
        if (iResult != ANN_NO_ERROR)
        {
            ...
            LOG("Decode info: layerIndex = %d, nodeIndex = %d, threshold = %lf",
                layerIndex, nodeIndex, tmpValue);
            retValue = iResult;
            goto RET;
        }
...
RET:
    if (strList != NULL)
    {
        ...
        {
            if (strList[i] != NULL) free(strList[i]);
        }
        free(strList);
    }
    ...
    if (childStrList != NULL)
    {
        ...
        {
            if (childStrList[i] != NULL) free(childStrList[i]);
        }
        free(childStrList);
    }
                               ...
                               struct ANN_FILE_BLOCK* fbPtr)
{
    ...
    // Checking
    iResult =
        ann_strcmp(fbPtr->header, ann_file_header[ANN_HEADER_RECURRENT_WEIGHT]);
    if (iResult != ANN_NO_ERROR)
    {
        retValue = ANN_INFO_NOT_FOUND;
        goto RET;
    ...
    for (i = 1; i < fbPtr->strCount; i++)
    {
        ...
        // Set weight
        iResult = ann_set_recurrent_weight_struct(asPtr, preNodeIndex - 1,
                                                  nodeIndex - 1, tmpValue);
        if (iResult != ANN_NO_ERROR)
        {
            retValue = iResult;
            goto RET;
...
int ann_parse_weight(struct ANN_STRUCT* asPtr, struct ANN_FILE_BLOCK* fbPtr)
{
    ...
    // Checking
    iResult =
        ann_strcmp(fbPtr->header, ann_file_header[ANN_HEADER_WEIGHT_FACTOR]);
    if (iResult != ANN_NO_ERROR)
    {
        retValue = ANN_INFO_NOT_FOUND;
        goto RET;
    ...
    for (i = 1; i < fbPtr->strCount; i++)
    {
        ...
        // Set weight
        iResult = ann_set_weight_struct(asPtr, layerIndex - 1, preNodeIndex - 1,
                                        nodeIndex - 1, tmpValue);
        if (iResult != ANN_NO_ERROR)
        {
            retValue = iResult;
...
int ann_parse_config(struct ANN_CONFIG_STRUCT* cfgPtr,
                     struct ANN_FILE_STRUCT* fsPtr)
{
    int iResult;
    int retValue = ANN_NO_ERROR;
...
RET:
    if (allocTmp != NULL) free(allocTmp);

    if (tFuncList != NULL) free(tFuncList);
                                   ...
                                   struct ANN_FILE_BLOCK* fbPtr)
{
    ...
    // Checking
    iResult =
        ann_strcmp(fbPtr->header, ann_file_header[ANN_HEADER_TRAINING_INFO]);
    if (iResult != ANN_NO_ERROR)
    {
        retValue = ANN_INFO_NOT_FOUND;
        goto RET;
