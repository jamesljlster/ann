...
int ann_allocate_network(struct ANN_STRUCT* sptr)
{
    ...
    for (i = 0; i < layers; i++)
    {
        ...
        else if (tFuncRoot == ANN_TFUNC_MULTIPLE)
        {
            ...
            {
                LOG("Checking failed, invalid transfer function id: %d",
                    tFuncList[i]);
                retValue = ANN_INVALID_ARG;
                goto ERR;
            }
            ...
            tmpLayer[i].dActiveFunc =
                ann_transfer_derivative_list[tFuncList[i]];
        }
        ...
        {
            LOG("Checking failed, invalid transfer function id: %d",
        tFuncList[i]); retValue = ANN_INVALID_ARG; goto ERR;
        }
        ...
        */
        ...
        else
        {
            ...
            for (j = 0; j < nodeList[i]; j++)
            {
                if (i > 0)
                {
                    allocTmp = calloc(nodeList[i - 1], sizeof(float));
                    if (allocTmp == NULL)
                    {
                        LOG("calloc failed with arg: nodeList[%d] = %d", i - 1,
                            nodeList[i - 1]);
                        retValue = ANN_MEM_FAILED;
                        goto ERR;
                    }
    ...
    if (layers > 2)
    {
        ...
        {
            allocTmp = calloc(nodeList[layers - 2], sizeof(float));
            if (allocTmp == NULL)
            {
                retValue = ANN_MEM_FAILED;
                goto ERR;
...
ERR:
    if (tmpLayer != NULL)
    {
        for (i = 0; i < layers; i++) ann_delete_layer(tmpLayer);
