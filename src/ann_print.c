...
void ann_print(ann_t ann) { ann_print_struct(ann, ANN_NETWORK_ANN); }

void rnn_print(ann_t ann) { ann_print_struct(ann, ANN_NETWORK_RNN); }
        ...
        for (k = 0; k < asPtr->layerList[i - 1].nodeCount; k++)
        {
            for (j = 0; j < asPtr->layerList[i].nodeCount; j++)
            {
                fprintf(fptr, "%02d-%02d-%02d=%.32G\n", i + 1, k + 1, j + 1,
                        asPtr->layerList[i].nodeList[j].weight[k]);
            }
        }
    }
    fprintf(fptr, "\n");
...
void ann_fprint_threshold(FILE* fptr, struct ANN_STRUCT* asPtr)
{
    ...
    for (i = 0; i < asPtr->config.layers; i++)
    {
        ...
        {
            fprintf(fptr, "%02d-%02d=%.32G\n", i + 1, j + 1,
                    asPtr->layerList[i].nodeList[j].threshold);
        }
    }
    fprintf(fptr, "\n");
...
void ann_fprint_topology(FILE* fptr, struct ANN_CONFIG_STRUCT* cfgPtr)
{
    ...
    fprintf(fptr, "%s= %d\n", ann_header_topology[ANN_HEADER_TOPOLOGY_INPUTS],
            cfgPtr->inputs);
    fprintf(fptr, "%s= %d\n", ann_header_topology[ANN_HEADER_TOPOLOGY_OUTPUTS],
            cfgPtr->outputs);
    fprintf(fptr, "%s= %d\n", ann_header_topology[ANN_HEADER_TOPOLOGY_LAYERS],
            cfgPtr->layers);
    fprintf(fptr, "%s=%s\n",
            ann_header_topology[ANN_HEADER_TOPOLOGY_TRANSFER_FUNC],
            ann_transfer_func_name[cfgPtr->tFuncRoot]);
    fprintf(fptr, "\n");
...
void ann_fprint_training_info(FILE* fptr, struct ANN_CONFIG_STRUCT* cfgPtr)
{
    ...
    fprintf(fptr, "%s= %G\n",
            ann_header_training_info[ANN_HEADER_TRAINING_INFO_LEARNING_RATE],
            cfgPtr->learningRate);
    fprintf(fptr, "%s= %G\n",
            ann_header_training_info[ANN_HEADER_TRAINING_INFO_MOMENTUM_COEF],
            cfgPtr->momentumCoef);
    fprintf(fptr, "\n");
