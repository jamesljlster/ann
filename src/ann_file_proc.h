...
extern "C"
{
    ...
    int ann_fstruct_create(struct ANN_FILE_STRUCT* fStructPtr,
                           const char* filePath);
    ...
    int ann_fstruct_append(struct ANN_FILE_STRUCT* dst,
                           struct ANN_FILE_BLOCK* src);
    ...
    int ann_str_extract(char*** strListPtr, int* strCountPtr, char* src,
                        char sepCh);
